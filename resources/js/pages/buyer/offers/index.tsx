...
export default function BuyerOffers({ offers, filters }: { offers: any; filters: { status?: string } }) {
    const handleStatusFilter = (value: string) => {
        router.get(buyer.offers.index().url, { ...filters, status: value }, { preserveState: true, replace: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Offers you've made to sellers</p>
                    </div>
                </div>
                                ...
                                {offers.data.length === 0 ? (
                                    ...
                                    offers.data.map((offer: Offer) => (
                                        <TableRow key={offer.id}>
                                            <TableCell className="font-medium">{offer.seller.full_name}</TableCell>
                                            ...
                                            <TableCell>
                                                <TableActions
                                                    actions={[
                                                        { label: 'View', href: buyer.offers.show(offer.id) },
                                                    ]}
                                                />
                                            </TableCell>
                                        </TableRow>
