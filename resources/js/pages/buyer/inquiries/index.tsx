...
export default function BuyerInquiries({ inquiries, filters }: { inquiries: any; filters: { search?: string; status?: string } }) {
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        router.get(buyer.inquiries.index().url, { ...filters, search: e.target.value }, { preserveState: true, replace: true });
    };

    const handleStatusFilter = (value: string) => {
        router.get(buyer.inquiries.index().url, { ...filters, status: value }, { preserveState: true, replace: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Questions you've sent to sellers</p>
                    </div>
                </div>
                                ...
                                {inquiries.data.length === 0 ? (
                                    ...
                                    inquiries.data.map((inquiry: Inquiry) => (
                                        <TableRow key={inquiry.id}>
                                            <TableCell className="font-medium">{inquiry.seller.full_name}</TableCell>
                                            ...
                                            <TableCell>
                                                <TableActions
                                                    actions={[
                                                        { label: 'View', href: buyer.inquiries.show(inquiry.id) },
                                                    ]}
                                                />
                                            </TableCell>
                                        </TableRow>
