...
export default function BuyerOrderShow({ order }: { order: OrderShow }) {
    const fmt = (n: number) =>
            ...
            <div className="mx-auto flex h-full w-full max-w-4xl flex-1 flex-col gap-5 p-5">
                <div className="no-print flex items-center justify-between">
                    <Button variant="ghost" size="sm" asChild>
                        <Link href={buyer.orders.index()} className="gap-2">
                    ...
                    <div className="grid grid-cols-1 gap-6 px-6 py-4 sm:grid-cols-2">
                        <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                                <p className="text-base font-semibold">{order.seller.dealer_name || order.seller.full_name}</p>
                                {order.seller.is_dealer && <p className="text-sm text-muted-foreground">{order.seller.full_name}</p>}
                                ...
                                <Button variant="outline" size="sm" asChild className="mt-2">
                                    <Link href={messages.create()} data={{ receiver_id: order.seller.id }}>
